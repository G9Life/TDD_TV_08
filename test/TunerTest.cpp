...
TEST(FakeTunerTest, SetAndGetChannel) {
  FakeTuner tuner({1, 3, 7});
...
TEST(FakeTunerTest, SeekNextAvailableChannel) {
  FakeTuner tuner({1, 3, 7});

  tuner.setCH("1");

  EXPECT_EQ("3", tuner.seekCH());
}

TEST(FakeTunerTest, SeekWrapsToFirstChannel) {
  FakeTuner tuner({1, 3, 7});
...
TEST(FakeTunerTest, ThrowsWhenChannelIsOutOfRange) {
  FakeTuner tuner({1, 3, 7});
